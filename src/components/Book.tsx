...
export default function Book() {
  ...
  /* Load react-pageflip dynamically (client-only, avoids SSR DOM issues) */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Map context pages → book pages:
    // context 0 → book 0 (cover)
    // context 1 → book 2 (proposal, spread [1,2])
    // context 2 → book 4 (date setup, spread [3,4])
    // context 3 → book 6 (receipt, spread [5,6])
    ...
    <div className="wood-background min-h-screen flex items-center justify-center p-4 md:p-8">
      <div className="relative w-full max-w-[380px] md:max-w-[720px] mx-auto">
        ...
        >
          ...
          <PageFrontCover />
          {/* Page 1: Left decorative */}
          <PageLeft />
          {/* Page 2: Proposal */}
          <PageProposal />
          {/* Page 3: Left decorative */}
          <PageLeft />
          {/* Page 4: Date Setup */}
          <PageDateSetup />
          {/* Page 5: Left decorative */}
          <PageLeft />
          {/* Page 6: Receipt */}
          <PageReceipt />
